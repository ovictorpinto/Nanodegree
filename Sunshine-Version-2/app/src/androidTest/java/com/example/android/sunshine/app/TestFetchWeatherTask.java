        ...
        for ( int i = 0; i < 2; i++ ) {
            ...
            if (locationCursor.moveToFirst()) {
                ...
                assertEquals("Error: the queried value of location setting is incorrect",
                        locationCursor.getString(locationCursor.getColumnIndex(WeatherContract.LocationEntry.COLUMN_LOCATION_SETTING)), ADD_LOCATION_SETTING);
                assertEquals("Error: the queried value of location city is incorrect",
                        locationCursor.getString(locationCursor.getColumnIndex(WeatherContract.LocationEntry.COLUMN_CITY_NAME)), ADD_LOCATION_CITY);
                assertEquals("Error: the queried value of latitude is incorrect",
                        locationCursor.getDouble(locationCursor.getColumnIndex(WeatherContract.LocationEntry.COLUMN_COORD_LAT)), ADD_LOCATION_LAT);
                assertEquals("Error: the queried value of longitude is incorrect",
                        locationCursor.getDouble(locationCursor.getColumnIndex(WeatherContract.LocationEntry.COLUMN_COORD_LONG)), ADD_LOCATION_LON);
            } else {
